...
<head>
    ...
    <title>Definir Nova Senha - SIGAT</title>
...
<body>
    <div class="login-card">
        <div class="text-center mb-4">
            <div
                ...
                <i class="fas fa-lock-open tw-text-2xl tw-text-white"></i>
            </div>
            <h2 class="tw-text-white tw-font-bold tw-text-2xl">Nova Senha</h2>
            <p class="tw-text-slate-400 tw-text-sm tw-mt-2">Crie uma nova senha de acesso para sua conta.</p>
        </div>
        ...
        <form id="resetForm">
            ...
            <div class="mb-3">
                <label class="tw-text-slate-300 tw-text-sm tw-mb-2 tw-block">Nova Senha</label>
                <input type="password" id="password" class="form-control form-control-dark"
                    placeholder="Mínimo 6 caracteres" required>
            </div>
            <div class="mb-4">
                <label class="tw-text-slate-300 tw-text-sm tw-mb-2 tw-block">Confirme a Senha</label>
                <input type="password" id="confirm" class="form-control form-control-dark"
                    placeholder="Repita a nova senha" required>
            </div>
            <button type="submit" class="btn btn-sigat w-100 mb-3" id="btnSubmit">
                <span id="btnText">Redefinir Senha</span>
                <span id="btnSpinner" class="tw-hidden"><i class="fas fa-spinner fa-spin tw-mr-2"></i>Salvando...</span>
            </button>
    ...
    <script>
        document.getElementById('resetForm').addEventListener('submit', async (e) => {
            ...
            try {
                const res = await fetch('api/forgot_password.php?action=reset', {
                    method: 'POST',
                ...
                if (data.success) {
                    alertBox.className = 'tw-block tw-bg-teal-500/20 tw-text-teal-300 tw-border tw-border-teal-500/30 tw-p-3 tw-rounded-xl tw-mb-4 tw-text-sm tw-text-center';
